...
<main class="container-admin">
    <h1 class="pt-5">Espace administrateur</h1><br>
    <img class="img_admin" src="src/image/admin.gif" alt="">
    <section>
        <br><h2 class="titre_admin">Gestion utilisateurs</h2>
        <center>
            <br><br>
            <table class="table1">
                <thead>
                ...
                <?php for($i = 0 ; $i < COUNT($resultat_users) ; $i ++) :?>
                    <tr>
                        ...
                        <td><a class="icon-edit" href="modification_user.php?id=<?=$resultat_users[$i][0] ?>" title="Modifier"><i class="fa-solid fa-pen-to-square"></i> </a> </td>
                        <td><button><a class="icon-trash" href="php/traitement/supprimer_user.php?id=<?= $resultat_users[$i][0]?>" title="supprimer" onclick="return confirm('Supprimer : <?=$resultat_users[$i]['login'];?> ?')"><i class="fa-solid fa-trash"></i></a></button></td>
                    </tr>
                <?php endfor ;?>
                </tbody>
            </table>
    ...
    <br><br><br>
    <section>
        <h2 class="titre_admin">Les articles</h2><br><br><br>
            ...
            <table class="table2">
                ...
                <?php for ($i = 0 ; $i<COUNT($resultat_recuperation_articles) ; $i++) :?>
                    <tr>
                        ...
                        <td><img src="php/traitement/upload/<?= $resultat_recuperation_articles[$i]['image'] ?>" class="image-boutique-admin" alt="Image"></td>
                        <td><?= $resultat_recuperation_articles[$i]['nom'] ?></td>
                        <td><?= $resultat_recuperation_articles[$i]['prix'] ?> €</td>
                        <td><a class="icon-edit" href="modification_article.php?id=<?= $resultat_recuperation_articles[$i][0]?>" title="Modifier"><i class="fa-solid fa-pen-to-square"></i></a></td>
                        <td><button><a class="icon-trash" href="php/traitement/supp_article.php?id=<?= $resultat_recuperation_articles[$i][0]?>" title="supprimer" onclick="return confirm('Supprimer : <?=$resultat_recuperation_articles[$i]['titre'];?> ?')"><i class="fa-solid fa-trash"></i></a></button></td>
        ...
        <?php if(isset($_GET['start'])) :?>
            <?php if ($_GET['start'] >= 5) : ?>
                ...
                <?php $offset = $offset - 5 ?>
                <a class="icon-left-circled" href="admin.php?start=<?= $offset ?>"><i class="fa-sharp fa-solid fa-arrow-left"></i></a>
        ...
        <?php if(isset($_GET['start'])) :?>
            <?php if($_GET['start'] < $nb_articles - 5) : ?>
                <?php $offset = $_GET['start'] ?>
                    <?php $offset = $offset + 5 ?>
                    <a  class="icon-right-circled" href="admin.php?start=<?= $offset  ?>"><i class="fa-sharp fa-solid fa-arrow-right"></i></a>
               
            <?php endif ;?>
        <?php else :?>
            <a class="icon-right-circled" href="admin.php?start=<?= $offset + 5?>"><i class="fa-sharp fa-solid fa-arrow-right"></i></a>
        <?php endif ;?>

        <br><br><br><br><h2 class="titre_admin">Catégories</h2><br>
        <center>
        <div class="div_admin">
            <table>
                <thead>
                ...
                <?php for ($i=0 ; $i <COUNT($categories) ; $i++ ):?>
                    <tr>
                        ...
                        <td><a class="icon-edit" href="modification_categorie.php?id=<?= $categories[$i]['id'] ?>" title="Modifier"><i class="fa-solid fa-pen-to-square"></i></a></td>
                        <td><button><a class="icon-trash" href="php/traitement/supprimer_categorie.php?id=<?= $categories[$i]['id'] ?>" title="supprimer" onclick="return confirm('Supprimer : <?=$categories[$i]['nom'];?> ?')"><i class="fa-solid fa-trash"></i></a></button></td>
                    </tr>
                <?php endfor ;?>
                </tbody>
            </table>
            ...
            <form action="php/traitement/ajout_categorie.php" method="POST">
                <br><br>
                <label class="">Ajouter une catégorie :</label>
                    
                <input class="" type="text" id="categorie" name="categorie" placeholder="Nom de la catégorie à ajouter">
