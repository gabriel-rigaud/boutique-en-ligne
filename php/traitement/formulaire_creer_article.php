    ...
    if (!empty($_POST['titre']) && !empty($_POST['categorie']) && !empty($_POST['article']) && !empty($_FILES) ) {
            ...
            if(!isset($erreur)) //S'il n'y a pas d'erreur, on upload
            {
                ...
                if(move_uploaded_file($_FILES['image']['tmp_name'], $dossier . $fichier)) //Si la fonction renvoie TRUE, c'est que ça a fonctionné...
                {
                    ...
                    $requete = $bd->prepare("INSERT INTO `articles`(`article`, `id_utilisateur`, `id_categorie`, `date`, `titre`, `image`, `prix`) VALUES (?, ?, ?, NOW(), ?, ?, ?)");
                    $requete->execute(array($article, $id_utilisateur, $categorie, $titre, $fichier, $prix));
