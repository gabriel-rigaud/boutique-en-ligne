    ...
    if (isset($_POST['valider'])) {
        ...
        if (!empty($titre) && !empty($article) && !empty($categorie)) {
            ...
            if (!empty($_FILES['image']['name'])) {
                ...
                $up_article = $bd->prepare("UPDATE `articles` SET `article`=?, `id_categorie`=?, `titre`=?, `image`=?, `prix`=? WHERE id = $id_article");
                $up_article->execute(array($article, $categorie, $titre, $fichier, $prix));
                header("location: ../../modification_article.php?id=$id_article");
                ...
                $up_article = $bd->prepare("UPDATE `articles` SET `article`=?, `id_categorie`=?, `titre`=?, `prix`=? WHERE id = $id_article");
                $up_article->execute(array($article, $categorie, $titre, $prix));
